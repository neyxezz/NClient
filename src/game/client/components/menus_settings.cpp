...
static CKeyInfo gs_aKeys[] =
{
	...
	{Localizable("Dummy reverse copy"), "toggle nc_dummy_reverse_copy_moves 0 1", 0, 0},
...
void CMenus::RenderSettingsControls(CUIRect MainView)
{
	...
	// dummy settings
	{
		...
		DummySettings.HSplitTop(123.0f, &DummySettings, &MiscSettings);
		if(s_ScrollRegion.AddRect(DummySettings))
		{
			...
			DoSettingsControlsButtons(29, 33, DummySettings);
		}
	}
	...
	// misc settings
	{
		...
		if(s_ScrollRegion.AddRect(MiscSettings))
		{
			...
			DoSettingsControlsButtons(33, 45, MiscSettings);
		}
	}
...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	const char *apTabs[SETTINGS_LENGTH] = {
		...
		else if(g_Config.m_UiSettingsPage == SETTINGS_FEATURES)
		{
			GameClient()->m_MenuBackground.ChangePosition(CMenuBackground::POS_SETTINGS_FEATURES);
			RenderSettingsFeatures(MainView);
		}
...
void CMenus::RenderSettingsFeatures(CUIRect MainView)
{
	CUIRect Button, Left, Right, LeftLeft, Label, EditBox;

	// Разделы Rapid Fire и Dummy
	CUIRect RapidFire, Dummy;
	MainView.HSplitTop(110.0f, &RapidFire, &MainView);
	RapidFire.VSplitMid(&Left, &Dummy, 20.0f);

	// Rapid Fire
	Left.HSplitTop(30.0f, &Label, &Left);
	Ui()->DoLabel(&Label, Localize("Rapid Fire"), 20.0f, TEXTALIGN_ML);

	Left.HSplitTop(5.0f, nullptr, &Left);

	Left.HSplitTop(20.0f, &Button, &Left);
	if(DoButton_CheckBox(&g_Config.m_NcRapidFire, Localize("Whether player is rapid firing (not implemented)"), g_Config.m_NcRapidFire, &Button))
	{
		g_Config.m_NcRapidFire ^= 1;
	}

	Left.HSplitTop(20.0f, &Button, &Left);
	if(DoButton_CheckBox(&g_Config.m_NcDummyRapidFire, Localize("Whether dummy is rapid firing"), g_Config.m_NcDummyRapidFire, &Button))
	{
		g_Config.m_NcDummyRapidFire ^= 1;
	}

	// Dummy
	Dummy.HSplitTop(30.0f, &Label, &Dummy);
	Ui()->DoLabel(&Label, Localize("Dummy"), 20.0f, TEXTALIGN_ML);

	Dummy.HSplitTop(5.0f, nullptr, &Dummy);

	Dummy.HSplitTop(20.0f, &Button, &Dummy);
	if(DoButton_CheckBox(&g_Config.m_NcDummyReverseCopyMoves, Localize("Enable reversed copymoves"), g_Config.m_NcDummyReverseCopyMoves, &Button))
		g_Config.m_NcDummyReverseCopyMoves ^= 1;

	if(g_Config.m_NcDummyReverseCopyMoves) {
		Dummy.HSplitTop(20.0f, &Button, &Dummy);
		if(DoButton_CheckBox(&g_Config.m_NcDummyReverseMovements, Localize("Reversed: reversed movements"), g_Config.m_NcDummyReverseMovements, &Button))
			g_Config.m_NcDummyReverseMovements ^= 1;
		Dummy.HSplitTop(20.0f, &Button, &Dummy);
		if(DoButton_CheckBox(&g_Config.m_NcDummyReverseTargetX, Localize("Reversed: reversed target x"), g_Config.m_NcDummyReverseTargetX, &Button))
			g_Config.m_NcDummyReverseTargetX ^= 1;
		Dummy.HSplitTop(20.0f, &Button, &Dummy);
		if(DoButton_CheckBox(&g_Config.m_NcDummyReverseTargetY, Localize("Reversed: reversed target y"), g_Config.m_NcDummyReverseTargetY, &Button))
			g_Config.m_NcDummyReverseTargetY ^= 1;
	}

	CUIRect ReplyAndOther;
	MainView.HSplitTop(220.0f, &ReplyAndOther, &MainView);
	ReplyAndOther.VSplitMid(&Left, &Right, 20.0f);

	// Раздел Reply
	CUIRect ReplyToPing;
	Left.HSplitTop(150.0f, &ReplyToPing, &Left);
	ReplyToPing.HSplitTop(30.0f, &Label, &ReplyToPing);
	Ui()->DoLabel(&Label, Localize("Messages"), 20.0f, TEXTALIGN_ML);
	ReplyToPing.HSplitTop(5.0f, nullptr, &ReplyToPing);

	ReplyToPing.HSplitTop(20.0f, &Button, &ReplyToPing);
	Button.VSplitMid(&LeftLeft, &Button);

	if(DoButton_CheckBox(&g_Config.m_NcEnableReplyPing, Localize("Enable reply"), g_Config.m_NcEnableReplyPing, &LeftLeft))
		g_Config.m_NcEnableReplyPing ^= 1;

	if(g_Config.m_NcEnableReplyPing)
	{
		ReplyToPing.HSplitTop(20.0f, &Label, &ReplyToPing);
		Label.VSplitLeft(100.0f, &Label, &EditBox);
		Ui()->DoLabel(&Label, Localize("Text"), 14.0f, TEXTALIGN_ML);

		EditBox.VSplitRight(5.0f, &EditBox, nullptr);
		static CLineInput s_ReplyPingInput(g_Config.m_NcReplyPing, sizeof(g_Config.m_NcReplyPing));
		s_ReplyPingInput.SetEmptyText(Localize("text..."));
		Ui()->DoEditBox(&s_ReplyPingInput, &EditBox, 14.0f);
	}

	ReplyToPing.HSplitTop(20.0f, &Button, &ReplyToPing);
	Button.VSplitMid(&LeftLeft, &Button);

	if(DoButton_CheckBox(&g_Config.m_NcEnableReplyPingNotTabbed, Localize("Enable not tabbed reply (not implemented)"), g_Config.m_NcEnableReplyPingNotTabbed, &LeftLeft))
		g_Config.m_NcEnableReplyPingNotTabbed ^= 1;

	if(g_Config.m_NcEnableReplyPingNotTabbed)
	{
		ReplyToPing.HSplitTop(20.0f, &Label, &ReplyToPing);
		Label.VSplitLeft(100.0f, &Label, &EditBox);
		Ui()->DoLabel(&Label, Localize("Text"), 14.0f, TEXTALIGN_ML);
		...
		EditBox.VSplitRight(5.0f, &EditBox, nullptr);
		static CLineInput s_ReplyPingInput(g_Config.m_NcReplyPingNotTabbed, sizeof(g_Config.m_NcReplyPingNotTabbed));
		s_ReplyPingInput.SetEmptyText(Localize("text..."));
		Ui()->DoEditBox(&s_ReplyPingInput, &EditBox, 14.0f);
	}

	ReplyToPing.HSplitTop(20.0f, &Button, &ReplyToPing);
	if(DoButton_CheckBox(&g_Config.m_NcEnableDeathMessage, Localize("Enable death message (not implemented)"), g_Config.m_NcEnableDeathMessage, &Button))
		g_Config.m_NcEnableDeathMessage ^= 1;

	if(g_Config.m_NcEnableDeathMessage)
	{
		ReplyToPing.HSplitTop(20.0f, &Label, &ReplyToPing);
		Label.VSplitLeft(100.0f, &Label, &EditBox);
		Ui()->DoLabel(&Label, Localize("Text"), 14.0f, TEXTALIGN_ML);
		...
		EditBox.VSplitRight(5.0f, &EditBox, nullptr);
		static CLineInput s_ReplyPingInput(g_Config.m_NcDeathMessage, sizeof(g_Config.m_NcDeathMessage));
		s_ReplyPingInput.SetEmptyText(Localize("text..."));
		Ui()->DoEditBox(&s_ReplyPingInput, &EditBox, 14.0f);
	}
	...
	// Раздел Other
	CUIRect Other;
	Left.HSplitTop(80.0f, &Other, &Left); // Выделяем место для Other
	Other.HSplitTop(30.0f, &Label, &Other);
	Ui()->DoLabel(&Label, Localize("Other"), 20.0f, TEXTALIGN_ML);

	Other.HSplitTop(5.0f, nullptr, &Other);

	Other.HSplitTop(20.0f, &Button, &Other);
	if(DoButton_CheckBox(&g_Config.m_NcFlagChatting, Localize("Whether to show chat bubble"), g_Config.m_NcFlagChatting, &Button))
		g_Config.m_NcFlagChatting ^= 1;

	Other.HSplitTop(20.0f, &Button, &Other);
	Ui()->DoScrollbarOption(&g_Config.m_NcParticlesAlpha, &g_Config.m_NcParticlesAlpha, &Button, Localize("Particles alpha"), 0, 100);
}
